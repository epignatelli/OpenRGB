...
void RGBController_NZXTHue1::SetupZones()
{
    ...
    | Set up zones                                      |
    \*-------------------------------------------------*/
    for(unsigned int zone_idx = 0; zone_idx < 2 /*controller->GetNumRGBChannels()*/; zone_idx++)
    {
        ...
        new_zone->leds_min      = 5;//0;
        new_zone->leds_max      = 5;//40;
        new_zone->leds_count    = 5;//controller->channel_leds[zone_idx];
...
void RGBController_NZXTHue1::SetCustomMode()
{
    active_mode = 0;
}
